...
def summary(ctx, device, json):
    ...
    # Check to see if we passwed in a device IP address or a device name
    try:
        ...
        if 'system-ip' in device_dict:
            ...
            system_ip = None
    omp_summary = mn.get_omp_summary(system_ip)
    if not json:
        pp = pprint.PrettyPrinter(indent=2)
        pp.pprint(omp_summary)
    if json:
        pp = pprint.PrettyPrinter(indent=2)
        pp.pprint(omp_summary)
@click.command()
